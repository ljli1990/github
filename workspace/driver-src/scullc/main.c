 ...
 */
static void scullc_do_deferred_op(struct work_struct *workp)
{
	//struct async_work *stuff = (struct async_work *) p;
	struct delayed_work *dwork = container_of(workp, struct delayed_work, work);
		...
		size_t count, loff_t pos)
{
	...
	INIT_WORK(&stuff->work.work, scullc_do_deferred_op);
...
{
	return scullc_defer_op(0, iocb, iov->iov_base, niov, pos);
}

static ssize_t scullc_aio_write(struct kiocb *iocb, const struct iovec *iov, unsigned long niov, loff_t pos)
{
	return scullc_defer_op(1, iocb, iov->iov_base, niov, pos);
}
